...
class LSBLZWBitUnpacker:
	...
	func get_bitv(value: int, index: int) -> int:
		return (value >> index) & 1

	func set_bitv(value: int, index: int) -> int:
	...
	func read_bits(bits_count: int) -> int:
		...
		for _i in range(bits_count):
			if self.get_bitv(byte, bit_index) == 1:
				result = self.set_bitv(result, result_bit_index)
			result_bit_index += 1
			bit_index += 1
