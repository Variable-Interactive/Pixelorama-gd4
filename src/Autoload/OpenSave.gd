...
func open_image_as_spritesheet_layer(
	...
	project.undo_redo.add_do_method(Callable(project, "add_frames").bind(frames, frame_indices))
	project.undo_redo.add_do_method(Callable(project, "add_layers").bind([layer], [project.layers.size()], [cels]))
	project.undo_redo.add_do_method(
		Callable(project, "change_cel").bind(new_frames_size - 1, project.layers.size())
	)
	project.undo_redo.add_do_method(Callable(Global, "undo_or_redo").bind(false))

	project.undo_redo.add_undo_method(Callable(project, "remove_layers").bind([project.layers.size()]))
	project.undo_redo.add_undo_method(Callable(project, "remove_frames").bind(frame_indices))
	project.undo_redo.add_undo_method(
...
func open_image_at_cel(image: Image, layer_index := 0, frame_index := 0) -> void:
	...
	project.undo_redo.add_do_method(Callable(project, "change_cel").bind(frame_index, layer_index))
	project.undo_redo.add_do_method(Callable(Global, "undo_or_redo").bind(false))
...
func open_image_as_new_frame(image: Image, layer_index := 0) -> void:
	...
	project.undo_redo.add_do_method(Callable(Global, "undo_or_redo").bind(false))
	project.undo_redo.add_do_method(Callable(project, "add_frames").bind([frame], [project.frames.size()]))
	project.undo_redo.add_do_method(Callable(project, "change_cel").bind(project.frames.size(), layer_index))

	project.undo_redo.add_undo_method(Callable(Global, "undo_or_redo").bind(true))
	project.undo_redo.add_undo_method(Callable(project, "remove_frames").bind([project.frames.size()]))
	project.undo_redo.add_undo_method(
		Callable(project, "change_cel").bind(project.current_frame, project.current_layer)
	)
	project.undo_redo.commit_action()
...
func open_image_as_new_layer(image: Image, file_name: String, frame_index := 0) -> void:
	...
	project.undo_redo.add_do_method(Callable(project, "add_layers").bind([layer], [project.layers.size()], [cels]))
	project.undo_redo.add_do_method(Callable(project, "change_cel").bind(frame_index, project.layers.size()))
...
func reload_backup_file(project_paths: Array, backup_paths: Array) -> void:
	...
	for i in range(backup_paths.size()):
		if FileAccess.file_exists(backup_paths[i]):
